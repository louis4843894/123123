...
function GuideContent() {
  ...
  const [currentStepId, setCurrentStepId] = useState<string | null>(
    ...
    initialType === 'preplanning' ? 'p1' :
    initialType === 'hasDirection' ? null : null
  )
  ...
  const stepIndex = getSteps().findIndex(s => s.id === currentStepId)
      ...
      {'message' in currentStep && currentStep.message ? (
        <div className="bg-card border border-border rounded-xl p-6">
          ...
          <Button
            ...
            onClick={() => handleMessageNext(currentStep.content.next)}
          >
