...
import type { DBVendor } from '@/lib/db'
...
function VendorCard({ vendor }: { vendor: DBVendor }) {
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden hover:shadow-md transition-all hover:-translate-y-0.5">
      <div className="h-36 bg-secondary/40 flex items-center justify-center">
        ...
        <div className="flex items-start justify-between mb-2">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {vendor.verified && <Shield size={13} className="text-accent" />}
            </div>
            {vendor.rating > 0 ? (
              ...
              <p className="text-xs text-muted-foreground">新進業者</p>
            )}
          </div>
        </div>
...
export function VendorsList({ vendors }: Props) {
  const allRegions = useMemo(() => [...new Set(vendors.flatMap(v => v.region))].sort(), [vendors])
  const allReligions = useMemo(() => [...new Set(vendors.flatMap(v => v.religion))].sort(), [vendors])
  ...
  const filtered = useMemo(() => {
    let result = [...vendors]
    if (regionFilter.length > 0) result = result.filter(v => v.region.some(r => regionFilter.includes(r)))
    if (religionFilter.length > 0) result = result.filter(v => v.religion.some(r => religionFilter.includes(r)))
    result.sort((a, b) => {
    ...
    return result
  }, [vendors, regionFilter, religionFilter, sortBy])

  const clearFilters = () => { setRegionFilter([]); setReligionFilter([]) }
      ...
      <div className="max-w-6xl mx-auto">
              ...
              <div className="flex flex-wrap gap-2">
                {allRegions.map(r => (
                  <button
                    ...
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${regionFilter.includes(r) ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-muted/40'}`}
                  >
                    {r}
                  </button>
                ))}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {allReligions.map(r => (
                  <button
                    key={r}
                    onClick={() => setReligionFilter(prev => prev.includes(r) ? prev.filter(x => x !== r) : [...prev, r])}
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${religionFilter.includes(r) ? 'bg-primary text-primary-foreground border-primary' : 'border-border hover:bg-muted/40'}`}
                  >
                    {r}
                  </button>
                ))}
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16">
            ...
            <button onClick={clearFilters} className="mt-3 text-sm text-accent hover:underline">清除所有篩選</button>
          </div>
        )}
      </div>
    </main>
