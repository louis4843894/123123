                ...
                {members.map(member => (
                  <div key={member.id} className="bg-card border border-border rounded-xl p-4 flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      {member.vote === 'approve' && (
                        <span className="text-xs bg-accent/10 text-accent px-2 py-1 rounded-full">已同意</span>
                      )}
                      {member.vote === 'reject' && (
                        <span className="text-xs bg-destructive/10 text-destructive px-2 py-1 rounded-full">反對</span>
                      )}
                      {member.vote === null && (
                        <span className="text-xs bg-muted text-muted-foreground px-2 py-1 rounded-full">待投票</span>
                      )}
                      {/* Demo: allow voting for pending members */}
                      {member.vote === null && (
                        <div className="flex gap-1 ml-2">
                          <button
                            onClick={() => handleVote(member.id, 'approve')}
                            className="w-7 h-7 rounded-lg bg-accent/10 hover:bg-accent/20 flex items-center justify-center transition-colors"
                          >
            ...
            <div className="bg-card border border-border rounded-xl flex flex-col" style={{ height: '520px' }}>
              ...
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
                {messages.map(msg => (
                  <div
                    key={msg.id}
                    className={`flex gap-3 ${msg.role === 'family' && msg.author.includes('大明') ? 'flex-row-reverse' : ''}`}
                  >
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-semibold shrink-0 ${
                      msg.role === 'vendor' ? 'bg-accent/20 text-accent' :
                      msg.role === 'system' ? 'bg-muted' : 'bg-primary/20 text-primary'
                    }`}>
                    ...
                    <div className={`max-w-xs lg:max-w-md ${msg.role === 'family' && msg.author.includes('大明') ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                      ...
                      <div className={`px-3 py-2 rounded-xl text-sm leading-relaxed ${
                        ...
                        msg.role === 'family' && msg.author.includes('大明')
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {members.map(m => (
                      <div key={m.id} className="flex items-center gap-3">
                        ...
                        <input
                          ...
                          max={100}
                          value={ratios[m.id]}
                          onChange={e => setRatios(prev => ({ ...prev, [m.id]: Number(e.target.value) }))}
                  ...
                  {splitAmounts().map(m => (
                    <div key={m.id} className="flex justify-between items-center py-2 border-b border-border last:border-0">
                      ...
                      <span className="text-sm font-semibold text-accent">
                        NT${m.amount.toLocaleString('zh-TW')}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-card border border-border rounded-xl p-5">
                ...
                <p className="text-xs text-muted-foreground mb-4 leading-relaxed">
                  靜途採用分段付款機制，確保每個服務步驟完成後才撥款，保障家屬權益。
                </p>
                {[
                  ...
                  <div key={stage.stage} className="flex items-center gap-3 py-2.5 border-b border-border last:border-0">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${
                      stage.status === 'paid' ? 'bg-accent' :
                      stage.status === 'pending' ? 'bg-amber-400' : 'bg-muted-foreground/30'
                    }`} />
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      stage.status === 'paid' ? 'bg-accent/10 text-accent' :
                      stage.status === 'pending' ? 'bg-amber-400/10 text-amber-600' :
