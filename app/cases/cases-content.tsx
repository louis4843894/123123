...
import { CheckCircle2, Circle, Bell, FileText, Upload } from 'lucide-react'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <div className="mb-3">
              ...
              <div className="h-3 bg-muted rounded-full overflow-hidden">
                <div className="h-full bg-accent rounded-full transition-all" style={{ width: `${paidRatio}%` }} />
              </div>
            ...
            <div className="flex flex-col gap-2 max-h-48 overflow-y-auto">
              {MOCK_MESSAGES.filter(m => m.role !== 'family').map(msg => (
                <div key={msg.id} className="flex gap-3 py-2 border-b border-border last:border-0">
                  <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${msg.role === 'vendor' ? 'bg-accent' : 'bg-amber-400'}`} />
                  <div>
          ...
          <div className="bg-card border border-border rounded-xl p-5">
            ...
            <div className="flex flex-col gap-2">
              {todos.map(todo => (
                <button key={todo.id} onClick={() => toggleTodo(todo.id)} className="flex items-start gap-3 text-left py-1.5">
            ...
            <div className="flex flex-col gap-2 mb-4">
              {[
                ...
                <div key={doc.name} className="flex items-center gap-3 py-2 border-b border-border last:border-0">
                  ...
                  {doc.status === 'pending' && (
                    <Button size="sm" variant="outline" className="text-xs ml-1 shrink-0">簽署</Button>
                  )}
                </div>
              ))}
            </div>
